...
const ec2 = new AWS.EC2({region: 'ap-northeast-1', apiVersion: '2016-11-15'})
  ...
  return new Promise(function(resolve, reject) {
    ec2.describeImages(params).promise()
      ...
      .then((data) => {
        ...
        resolve({Region: region, Name: images[0].ImageId})
      }); 
    })
}
...
function listLatestAMIForRegions(amiName) {
  return new Promise((resolve, reject) => {
    ...
    .then((regions) => {
      ...
      let regionAMIMap=regions.map((region) =>  { return getImageForRegion(region,  amiName)})
